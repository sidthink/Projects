...
static void wbrand(const Ann t){
...
//forward propagation
static void fprop(const Ann t, const float* const in){
...
static void bprop(const Ann t, const float* const in, const float* const tg, float rate){
...
float* xtpredict(const Ann t, const float* const in){
    fprop(t,in);
    return t.o;
}

float xttrain(const Ann t, const float* const in , const float* const tg, float rate){
    fprop(t, in);
    bprop(t, in, tg, rate);
    return toterr(tg, t.o, t.nops);
}

//Initialise ANN with given inputs, hidden layer(1) neurons and outputs.
Ann xtbuild(const int nips, const int nhid, const int nops){
    Ann t;
    t.nb = 2;
    t.nw = nhid*(nips +nops);
    ...
    t.nhid = nhid;
    t.nops = nops;
    wbrand(t); // to randomise weights and biases
    return t;

}

// Frees object from heap.
void xtfree(const Ann t)
{
    free(t.w);
    free(t.b);
    free(t.h);
...
void xtsave(const Ann t, const char* const path)
{
...
Ann xtload(const char* const path)
{
    ...
    fscanf(file, "%d %d %d\n", &nips, &nhid, &nops);
    // Build a new tinn.
    const Ann t = xtbuild(nips, nhid, nops);
